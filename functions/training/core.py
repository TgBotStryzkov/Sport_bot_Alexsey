...
async def choose_program(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if мышцы:
        await query.edit_message_text(text="Теперь выбери мышцу:", reply_markup=get_muscle_keyboard(мышцы))
    else:
...
async def choose_muscle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    from utils.keyboard import get_full_exercise_keyboard  # убедись, что импорт есть
